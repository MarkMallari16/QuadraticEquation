...
quadraticEquationElement.textContent = '\\(ax^2 + bx + c = 0\\)';
...
function calculate() {
    ...
    quadraticEquationFormulaResultElement.textContent = `Solution = ${quadraticEquationFormulaResult}`;
    ...
    if (discriminant > 0) {
        ...
        result.innerHTML = `Final Answer = \\\[ \\text{Roots: } x_1 = \\frac{${-b} + \\sqrt{${discriminant}}}{${2 * a}}, \\ x_2 = \\frac{${-b} - \\sqrt{${discriminant}}}{${2 * a}} \\]`;
        MathJax.typeset([result]);
    } else if (discriminant === 0) {
        const root = -b / (2 * a);
        result.innerHTML = `Final Answer = Root: x = ${root}`;
    } else {

        result.innerHTML = `Final Answer: <span class="highlighted-answer">No real roots</span>`;
    }
